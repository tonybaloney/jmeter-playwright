...
class PlaywrightTestThreadGroupGUI : AbstractThreadGroupGui() {
    ...
    private fun init() {
        ...
        // Test folder
        val testFolderPanel = JPanel(BorderLayout(5, 0))
        val testFolderLabel = JLabel("Playwright Test Folder") // $NON-NLS-1$
        testFolderPanel.add(testFolderLabel, BorderLayout.WEST)
        testDirectory.fileSelectionMode = JFileChooser.DIRECTORIES_ONLY
        testDirectory.name = THREAD_NAME
        testFolderLabel.labelFor = testDirectory
        testFolderPanel.add(testDirectory, BorderLayout.CENTER)
        threadPropsPanel.add(testFolderPanel)
    ...
    override fun getLabelResource(): String {
        return "Playwright Test Thread Group"
    }
    ...
    override fun createTestElement(): TestElement {
        val threadGroup = PlaywrightTestThreadGroup()
